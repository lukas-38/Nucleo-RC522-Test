...
uchar getdata[16];
int l, ii;
uchar senddata[] = { 52, 49, 46, 46, 70, 24, 0, 0, 57, 41, 56, 45, 66, 65, 74,
		66 };
char reset[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	while (1) {
		...
		if (status == MI_OK) {
			...
			sprintf((char*) mystr, "%3d:%3d:%3d:%3d\n\r", (uchar) str[0],
					(uchar) str[1], (uchar) str[2], (uchar) str[3]);
				...
				if (process == MI_OK) {
					...
					if (process == MI_OK) {
						...
						if (process == MI_OK) {
							...
							//		SER_putString (0,":");
							for (l = 0; l < 16; l++) {
								HAL_UART_Transmit(&huart2, (uchar*) &getdata[l],
										1, 5000);
								HAL_UART_Transmit(&huart2, (uchar*) ":", 1,
										5000);
							}
